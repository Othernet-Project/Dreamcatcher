 ...
 * Unused currently in favor of json
 */
static esp_err_t http_resp_dir_html(httpd_req_t *req, const char *dirpath)
{
    ...
    extern const unsigned char files_tree_start[] asm("_binary_files_tree_html_start");
    extern const unsigned char files_tree_end[]   asm("_binary_files_tree_html_end");
    const size_t files_tree_size = (files_tree_end - files_tree_start);

    /* Add file upload form and script which on execution sends a POST request to /upload */
    httpd_resp_send_chunk(req, (const char *)files_tree_start, files_tree_size);
    ...
    // ignore in top folder (/files/)
    if (strlen(dirpath) > 7)
    {
        ESP_LOGI(TAG, "path: %s", dirpath);
        httpd_resp_sendstr_chunk(req, "<tr><td><a href=\"");
        httpd_resp_sendstr_chunk(req, dirpath);        
        httpd_resp_sendstr_chunk(req, "../");
        httpd_resp_sendstr_chunk(req, "\">");
        httpd_resp_sendstr_chunk(req, "Up");
        httpd_resp_sendstr_chunk(req, "</a></td><td>");
        httpd_resp_sendstr_chunk(req, "directory");
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        if(n >= queryindex && n < queryindex + 10)
        {
    ...
    httpd_resp_sendstr_chunk(req, itoa(n, buff, 10));
